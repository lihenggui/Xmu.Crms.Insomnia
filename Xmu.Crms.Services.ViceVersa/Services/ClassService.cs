...
namespace Xmu.Crms.Services.ViceVersa
{
    class ClassService : IClassService
    {
        ...
        public long getTeacherIdByClassId(int classId) //add
        {
            return _classDao.getTeacherIdByClassId(classId);
            //if (classId <= 0)
            //{
            //    throw new ArgumentException();
            //}
            //var teacher = (from i in _db.ClassInfo
            //               from j in _db.Course
            //               where i.Course.Id == j.Id && i.Id == classId
            //               select j.Teacher.Id).SingleOrDefault();
            //return teacher;
        }

        public IList<ClassInfo> listClassByUserId(long userId)//add
        {
            return _classDao.listClassByUserId(userId);
            //if (userId <= 0)
            //{
            //    throw new ArgumentException();
            //}
            //var class1 = (from i in _db.CourseSelection
            //              from j in _db.ClassInfo
            //              where i.Student.Id == userId && j.Id == i.ClassInfo.Id
            //              select new ClassInfo
            //              {
            //                  Id = j.Id,
            //                  Name = j.Name,
            //                  Course = (from c1 in _db.Course
            //                            where c1.Id == j.Course.Id
            //                            select c1).SingleOrDefault(),
            //                  Site = j.Site,
            //                  ClassTime = j.ClassTime,
            //                  ReportPercentage = j.ReportPercentage,
            //                  PresentationPercentage = j.PresentationPercentage,
            //                  FivePointPercentage = j.FivePointPercentage,
            //                  FourPointPercentage = j.FourPointPercentage,
            //                  ThreePointPercentage = j.ThreePointPercentage
